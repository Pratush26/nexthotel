...
export default function ExploreClient({ filters, filteredRooms, currentType }) {
    ...
    // Initialize form with react-hook-form, default values from URL params
    ...
    const onSubmit = (data) => {
        ...
        if (data.checkIn)
            params.set(
                "checkIn",
                data.checkIn.toISOString().split("T")[0] // yyyy-mm-dd format
            );
        if (data.checkOut)
            params.set(
                "checkOut",
                data.checkOut.toISOString().split("T")[0] // yyyy-mm-dd format
            );
    ...
    return (
        <>
            ...
            <div className="flex flex-wrap justify-evenly w-full gap-3 mb-4">
                <div className="flex justify-center items-center">
                    <span className="flex flex-wrap justify-center items-center gap-2">
                        {filters.map(({ label, value }) => {
                            const href = value
                                ? `/cage/check-bookings?type=${encodeURIComponent(value)}${searchParams.get("checkIn") && searchParams.get("checkOut")
                                    ? `&checkIn=${searchParams.get("checkIn")}&checkOut=${searchParams.get("checkOut")}`
                                    : ""
                                }`
                                : `/cage/check-bookings${searchParams.get("checkIn") && searchParams.get("checkOut")
                                    ? `?checkIn=${searchParams.get("checkIn")}&checkOut=${searchParams.get("checkOut")}`
                                    : ""
                                }`;
                            const isActive =
                                (!value && !watchType) ||
                                (value?.toLowerCase() === watchType?.toLowerCase());
                            return (
                                <Link
                                    key={label}
                                    href={href}
                ...
                {/* Date Filter Form with react-hook-form Controller & react-datepicker */}
                <form
                        ...
                        <Controller
                            ...
                            render={({ field }) => (
                                <DatePicker
                                    ...
                                    minDate={
                                        watch("checkIn") ? new Date(watch("checkIn").getTime() + 86400000) : new Date()
                                    }
            ...
            <div className="grid grid-cols-1 justify-center items-center sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8 p-6">
                {filteredRooms.map((room) => (
                    <div
                        key={room._id}
                        className={`flex flex-col justify-center items-center gap-2 hover:border-1 hover:shadow-none border-gray-300 rounded-lg p-4 w-full shadow-gray-50/60 shadow-[0_1px_8px_rgba(0,0,0,0.3)] transition-all duration-300 ${room.bookedDate.length > 0 ? "bg-emerald-950" : ""
                            }`}
                    >
